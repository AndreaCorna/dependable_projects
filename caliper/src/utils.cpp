    ...
    if (confFile.is_open()) {
        ...
        while (!confFile.eof()) {
            if (line != "") {
                ...
                config = tokens[0];
                if (config.find(PERIOD_STR) != std::string::npos) {
                    period = atoi(config.substr(config.find(PERIOD_STR) + strlen(PERIOD_STR)).c_str());
                    config = config.substr(0, config.find(PERIOD_STR));
                } else {
                    period = NOT_DEF_DURATION;
                }
                ...
                if (config == EMPTY_SET) {
                    ...
                    init_num_of_cores = (int) tokens.size() - 1; //-1 since we have to discard the configuration string
                    initNotRead = false;
                ...
                if (tokens.size() - 1 != init_num_of_cores) {
                    std::cerr << "Line " << lineNumber << " - Missing data" << std::endl;
                    exit(1);
                }
                ...
                //  exit(1);
                //}
                configurations[config].push_back(aging_config_t(period, dagings));
                dagings.clear();
